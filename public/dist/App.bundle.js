...
function autocomplete(input, latInput, lngInput) {
    ...
    dropdown.addListener('place_changed', function () {
        var place = dropdown.getPlace(); // Este metodo retorna un objeto con toda la información sobre el lugar
...
function ajaxHeart(e) {
    ...
    _axios2.default.post(this.action).then(function (res) {
        var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // Se puede acceder a .heart porque es el nombre que tiene un elemento dentro de la etiqueta form (en este caso un boton)
...
function loadPlaces(map) {
    ...
    _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
        ...
        // Create a bounds
        var bounds = new google.maps.LatLngBounds(); // Esto sirve para que el mapa se acomode a la cantidad de marcadores que habrán en el mapa
        ...
        // When someone clicks on a marker, show the detail of that place
...
function makeMap(mapDiv) {
    ...
    var autocomplete = new google.maps.places.Autocomplete(input); // Crea un autocomplete en el elemento DOM dado
...
// libreria para evitar ataques XSS
function searchResultsHTML(stores) {
    return stores.map(function (store) {
        return '\n        <a href="/store/' + store.slug + '" class="search__result">\n            <strong>' + store.name + '</strong>\n        </a>\n    ';
    }).join('');
} // libreria como un "fetch" pero con algunas mejoras
...
function typeAhead(search) {
    ...
    searchInput.on('keyup', function (e) {
        ...
        var next = void 0; // Para saber cual será el siguiente en la lista a seleccionar

        // navigation behavior
        if (e.keyCode === 40 && current) next = current.nextElementSibling || items[0]; // items[0] en caso de que el seleccionado en ese momento sea el ultimo en la lista
        else if (e.keyCode === 40) // En el caso que no haya ninguno seleccionado aún
                next = items[0];else if (e.keyCode === 38 && current) next = current.previousElementSibling || items[items.length - 1];else if (e.keyCode === 38) next = items[items.length - 1];else if (e.keyCode === 13 && current.href) window.location = current.href;
